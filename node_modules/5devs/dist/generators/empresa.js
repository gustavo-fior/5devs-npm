...
exports.dataAbertura = dataAbertura;
exports.site = site;
exports.email = email;
const inscricoes_estaduais_1 = require("../data/inscricoes-estaduais");
...
function inscricaoEstadual(mask) {
    const inscricaoEstadual = inscricoes_estaduais_1.inscricoes[Math.floor(Math.random() * inscricoes_estaduais_1.inscricoes.length)];
    if (mask) {
        return inscricaoEstadual;
    }
    return inscricaoEstadual?.replace(/[\/\.\s-]/g, "");
}
function companyData(mask) {
    const prefixes = [
        "Cia",
        "Grupo",
        "Indústria",
    ...
    return {
        razaoSocial: showPrefix
            ? `${prefix} ${name} ${activity}`
            : `${name} ${activity}`,
        cnpj: cnpj(mask),
...
function site(name, activity) {
    const domain = [".com", ".com.br", ".net"];
    const domainName = domain[Math.floor(Math.random() * domain.length)];
    return `www.${name.toLowerCase()}${(0, utils_1.cleanString)(activity.toLowerCase())}${domainName}`;
}
function email(name) {
    const initial = [
        "contato",
