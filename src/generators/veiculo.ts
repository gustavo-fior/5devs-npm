...
export const vehicle = (mask?: boolean) => {
  const veiculo = veiculos[Math.floor(Math.random() * veiculos.length)];
  const estado =
    estados.estados[Math.floor(Math.random() * estados.estados.length)];
  const municipio =
    estado?.cidades[Math.floor(Math.random() * estado.cidades.length)];
  ...
  return {
    ...
    combustivel:
      combustiveis[Math.floor(Math.random() * combustiveis.length)] ??
